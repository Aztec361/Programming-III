...
public class CS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
      ...
      Vertex newVert = vertices.get(start);
      Vertex endVert = vertices.get(end);
      Path startPath =  new Path(newVert);
      PriorityQueue<Path> newQueue = new PriorityQueue<Path>();
      newQueue.add(startPath);
    
      
      while(!newQueue.isEmpty()){
        Path  remPath = newQueue.remove();
        if(remPath.end.equals(endVert)){
          return remPath;
        }
        newList.add(remPath.end);
        for(Edge newEdge :remPath.end.edgesLeaving){
          if(newList.contains(newEdge.target) == true){
            continue;
          }
          else{
            Path newPath = new Path(remPath,newEdge);
            newQueue.add(newPath);
          }
        }
        
      }
      
       throw  new NoSuchElementException(); // TODO: Implement this method in Step 7.
    }
